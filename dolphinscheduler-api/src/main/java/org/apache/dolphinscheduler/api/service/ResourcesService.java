...
public class ResourcesService extends BaseService {
    ...
    public Result createDirectory(User loginUser,
        ...
        try {
            int id = resourcesMapper.getMaxId();
            resource.setId(id);
    ...
    /**
     * query resource by name and type
     * @param loginUser login userS
     * @param fullName  resource full name
     * @param type      resource type
     * @return true if the resource name not exists, otherwise return false
     */
    public Result queryResourceByFullName(String fullName, ResourceType type,User loginUser) {
        Result result = new Result();
        if (StringUtils.isBlank(fullName) ) {
            logger.error("You must input fullName ");
            putMsg(result, Status.REQUEST_PARAMS_NOT_VALID_ERROR);
            return result;
        }
        List<Resource> resourceList = resourcesMapper.queryResourceList(fullName, loginUser.getId(), type.ordinal());
    ...
    /**
     * verify resource by id
     * @param id        resource id
     * @return true if the resource id not exists, otherwise return false
     */
    public Result queryResourceById(Integer id) {
        Result result = new Result();
        if (id == null || id.intValue() <= 0) {
            logger.error("id cannot be empty or less than or equal to zero!");
            putMsg(result, Status.REQUEST_PARAMS_NOT_VALID_ERROR);
            return result;
        }
        Resource resource = resourcesMapper.selectById(id);
    ...
    public Result onlineCreateResource(User loginUser, ResourceType type, String fileName, String fileSuffix, String desc, String content,int pid,String currentDirectory) {
        ...
        resource.setId(resourcesMapper.getMaxId());
        resourcesMapper.insert(resource);
