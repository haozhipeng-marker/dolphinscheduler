    ...
    public void executeFuncAndSql(SqlBinds mainSqlBinds,
        ...
        try {
            ...
            if (sqlParameters.getSqlType() == SqlType.QUERY.ordinal()) {
                ...
                // non query statement
                if(mainSqlBinds.getSql().contains("alter") || mainSqlBinds.getSql().contains("ALTER")){
                    String sql = mainSqlBinds.getSql();
                    Map<Integer, Property> params = mainSqlBinds.getParamsMap();
                    if(params != null) {
                        for (Map.Entry<Integer, Property> entry : params.entrySet()) {
                            Property prop = entry.getValue();
                            sql = sql.replaceFirst("\\?",prop.getValue());
                        }
                    }
                    stmt.execute(sql);
                } else {
