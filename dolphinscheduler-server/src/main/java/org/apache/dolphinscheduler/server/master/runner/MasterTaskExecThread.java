...
public class MasterTaskExecThread extends MasterBaseTaskExecThread {
    ...
    public Boolean submitWaitComplete() {
        ...
        logger.info("task :{} id:{}, process id:{}, exec thread completed ", this.taskInstance.getName(),taskInstance.getId(), processInstance.getId() );
        return result;
    }
    ...
    public Boolean waitTaskQuit(){
        ...
        logger.info("wait task: process id: {}, task id:{}, task name:{} complete", this.taskInstance.getProcessInstanceId(), this.taskInstance.getId(), this.taskInstance.getName());
        // task time out
            ...
            try {
                ...
                if(checkTimeout){
                    ...
                    if (remainTime < 0) {
                        ...
                        // send mail alert
                        alertDao.sendTaskTimeoutAlert(processInstance.getWarningGroupId(), processDefine.getReceivers(), processDefine.getReceiversCc(), processInstance.getId(), processInstance.getName(), taskInstance.getId(), taskInstance.getName());
                        // send sms alert
                        alertDao.sendSmsTaskTimeoutAlert(processInstance, processDefine, taskInstance);
                        checkTimeout = false;
                    }
                }
                ...
                if (processInstance != null) {
                    logger.error("wait task quit failed, instance id:{}, task id:{}", processInstance.getId(), taskInstance.getId());
                }
            }
        }
        return true;
